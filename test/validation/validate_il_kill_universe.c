...
int32_t main(void) {
	...
	if (validate_il(&input)) {
		fprintf(stderr, "IL tree invalidation test when attempting to kill a universe failed.\n");
		exit(EXIT_SUCCESS); 
	} else {
		fprintf(stderr, "IL tree invalidation test when attempting to kill a universe succeeded.\n");
		exit(EXIT_FAILURE);
	}
}
